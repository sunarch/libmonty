...
def bytes_per_line(kwargs: dict, args: list[str], args_index: int) -> int:
    ...
    if b_process_bytes_per_line:
        print(f'Bad value for \'byte count per line\': \'{s_bytes_per_line}\'')
...
def sleep(kwargs: dict, args: list[str], args_index: int) -> float:
    ...
    if b_process_sleep:
        try:
            ...
            print('Bad value for \'sleep\': \'{s_sleep}\'')

            s_confirm = input(f'Do you wish to continue with the default [{i_sleep}] ? (y/n) ')
            if s_confirm.lower() not in ("y", "yes"):
                raise ValueError(f'Bad value for \'sleep\': \'{s_sleep}\'')
    ...
    try:
        ...
        if len(args) > args_index:
            ...
            try:
                ...
                err = 'Value for index format not recognized: \'{s_index_converter}\''
                print(err)
