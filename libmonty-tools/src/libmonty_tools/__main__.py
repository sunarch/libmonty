...
# imports: project group
import libmonty_hexer.__main__
import libmonty_hexer.arguments
...
def main() -> None:
    ...
    parser_hexer = subparsers.add_parser('hexer', help='a hex dump tool')
    libmonty_hexer.arguments.create_arguments(parser_hexer)
    ...
    if 'command' not in vars(args):
        ...
        try:
            if args.command == 'hexer':
                libmonty_hexer.__main__.main_lib(args)
